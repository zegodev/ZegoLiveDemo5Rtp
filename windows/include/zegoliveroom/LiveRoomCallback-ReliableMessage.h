    ...
    namespace LIVEROOM
    {
        ...
        public:
            ...
            /**
             ...
             @param msgSeq      业务服务端最新序号
             @param pszUserId   发送该业务广播的用户ID
             @param pszUsername 发送该业务广播的用户名
             @param sendTime    发送时间
             */
            virtual void OnGetReliableMessage(int errorCode, const char *pszRoomID, int sendSeq, const char *pszMsgType, const char *pszMsgData, unsigned int msgSeq, const char *pszUserId, const char *pszUsername, unsigned long long sendTime) {}
